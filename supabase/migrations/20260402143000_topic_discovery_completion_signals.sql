...
with grouped as (
  select
    ...
    coalesce(
      sum(
        case
          when event_type = 'lesson_completed' then greatest(0, coalesce((metadata ->> 'reteachCount')::numeric, 0))
          else 0
        end
      ),
      0
    )::integer as lesson_complete_reteach_total,
    count(*) filter (
    ...
    coalesce(
      sum(
        case
          when event_type = 'lesson_completed'
            and created_at >= now() - interval '30 days'
            then greatest(0, coalesce((metadata ->> 'reteachCount')::numeric, 0))
          else 0
        end
      ),
      0
    )::integer as recent_lesson_complete_reteach_total,
...
select
  ...
  lesson_complete_reteach_total,
  recent_lesson_complete_reteach_total,
  sample_size,
  case
